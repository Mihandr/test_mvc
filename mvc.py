...
class MinesweeperModel:
    ...
    def openCell( self, row, column ):
        ...
        cell.counter = self.mock_countMinesAroundCell( row, column )
        if cell.counter == 0:
        ...
        for i in range( self.mineCount ):
            while True:
                ...
                if not cell.state == 'opened' and not cell.mined:
                    cell.mined = True
                    break
    def mock_countMinesAroundCell( self, row, column ):
        return TEST
        ...
        for row in range( self.model.rowCount ):
            ...
            for column in range( self.model.columnCount ):
                btn = Button(
                    ...
                    width = 2,
                    height = 1,
                    command = lambda row = row, column = column: self.controller.onLeftClick( 3, 3), # (row, column)
                    padx = 0,
                    pady = 0
                )
                btn.pack( side = LEFT )
                btn.bind(
                    '<Button-3>',
                    lambda e, row = row, column = column: self.controller.onRightClick(3, 3)
                )
